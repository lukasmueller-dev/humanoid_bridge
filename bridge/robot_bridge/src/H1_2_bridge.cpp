...
sairol_bridge::H1_2Bridge::H1_2Bridge(rclcpp::Node::SharedPtr node) : BridgeCore(node)
{
    ...
    lowStateSubscriber_ = nh->create_subscription<unitree_hg::msg::LowState>(
        "/lowstate", 1, std::bind(&sairol_bridge::H1_2Bridge::lowStateHandler_, this, std::placeholders::_1));

    remoteControlSubscriber_ = nh->create_subscription<unitree_go::msg::WirelessController>(
        "/wirelesscontroller", 10, std::bind(&sairol_bridge::H1_2Bridge::wireless_callback, this, std::placeholders::_1));
    ...
    lowCommandPublisher_ = nh->create_publisher<unitree_hg::msg::LowCmd>("/lowcmd", 10);
...
bool sairol_bridge::H1_2Bridge::checkExternalPublisher_(std::string topic_name)
{
...
void sairol_bridge::H1_2Bridge::stop()
{
    ...
    RCLCPP_INFO(nh->get_logger(), "H1-2 Bridge stopped.");
}

void sairol_bridge::H1_2Bridge::wireless_callback(unitree_go::msg::WirelessController::SharedPtr data)
{
    uint32_t key = data->keys;

    if ((key & (KEY_L2 | KEY_UP | KEY_LEFT)) == (KEY_L2 | KEY_UP | KEY_LEFT))
    {
...
void sairol_bridge::H1_2Bridge::lowStateHandler_(unitree_hg::msg::LowState::SharedPtr msg)
{
    if (mode_machine_ != msg->mode_machine)
    {
        if (mode_machine_ == 0)
        {
            RCLCPP_INFO(nh->get_logger(), "H1-2 type: %d", unsigned(msg->mode_machine));
        }
    ...
    const auto motor_count = std::min(static_cast<size_t>(numJoint_), msg->motor_state.size());
...
void sairol_bridge::H1_2Bridge::publishLowCommand_()
{
    ...
    unitree_cmd.mode_pr = H1_2_PRorAB::H1_2_PR;
    ...
    for (int i = 0; i < numJoint_; ++i)
    {
        ...
        unitree_cmd.motor_cmd[i].mode = 1;
...
bool sairol_bridge::H1_2Bridge::initControl_(bridge_interface::msg::RobotCmd default_cmd)
{
    last_state_time_ = nh->get_clock()->now();
...
void sairol_bridge::H1_2Bridge::finishControl_()
{
    RCLCPP_INFO(nh->get_logger(), "finishControl_ called from H1_2Bridge");
}
