...
def main():
    ...
    check("received /lowcmd", len(samples) > 1000, "{} samples".format(len(samples)))
    if not samples:
        sys.exit(1)

    span = samples[-1]["t"] - samples[0]["t"]
    rate = len(samples) / span if span else 0.0
    check("publishes at ~1 kHz", 0.8 * BRIDGE_HZ < rate < 1.2 * BRIDGE_HZ,
          "{:.0f} Hz".format(rate))

    t0 = samples[0]["t"]
    ramped = min(samples, key=lambda s: abs((s["t"] - t0) - (RAMP_SECONDS + 0.3)))
    check("start_control honoured",
          all(abs(v - RAMP_DEFAULT) < 1e-2 for v in ramped["q"][:n]),
          "q[0]={:.4f} wanted {}".format(ramped["q"][0], RAMP_DEFAULT))

    last = samples[-1]
    want = fingerprint(n)
    check("joint j lands on motor j",
          all(abs(a - b) < 1e-3 for a, b in zip(last["q"], want)),
          "q[:3]={}".format([round(v, 4) for v in last["q"][:3]]))
    check("kp from config", all(abs(a - b) < 1e-3
                                for a, b in zip(last["kp"], config["MOTOR_KP"])))
    check("kd from config", all(abs(a - b) < 1e-3
                                for a, b in zip(last["kd"], config["MOTOR_KD"])))
