...
class RemoteControlService:
    ...
    def _start_keyboard_thread(self):
        try:
            ...
            self.keyboard_runner = threading.Thread(target=self._keyboard_listener_wrapper)
            self.keyboard_runner.daemon = True
            self.keyboard_runner.start()
        except Exception as e:
            print(f"Failed to start keyboard thread: {e}")
            self._keyboard_listener_active = False

    def _keyboard_listener_wrapper(self):
        """Wrapper to handle keyboard listener with proper cleanup."""
        try:
            listen_keyboard(self._handle_keyboard_press)
        except Exception as e:
    ...
    def close(self):
        """Clean up resources."""
        print("Cleaning up RemoteControlService...")
        self._running = False
        
        # Stop keyboard listener first
        if self._keyboard_listener_active:
            try:
                stop_listening()
                self._keyboard_listener_active = False
        ...
        if hasattr(self, "joystick_runner") and getattr(self, "joystick_runner") != None:
            try:
                ...
                print("Joystick thread joined.")
            except Exception as e:
                print(f"Error joining joystick thread: {e}")
                
        if hasattr(self, "keyboard_runner") and getattr(self, "keyboard_runner") != None:
            try:
                self.keyboard_runner.join(timeout=1.0)
                print("Keyboard thread joined.")
            except Exception as e:
                print(f"Error joining keyboard thread: {e}")
