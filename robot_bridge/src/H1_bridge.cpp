...
sairol_bridge::H1Bridge::H1Bridge(rclcpp::Node::SharedPtr node) : BridgeCore(node)
{
    ...
    lowStateSubscriber_ = nh->create_subscription<unitree_go::msg::LowState>(
        "/lowstate", 1, std::bind(&sairol_bridge::H1Bridge::lowStateHandler_, this, std::placeholders::_1));

    // remoteControlSubscriber_ = nh->create_subscription<unitree_go::msg::WirelessController>(
    //     "/wirelesscontroller", 10, std::bind(&sairol_bridge::H1Bridge::wireless_callback, this, std::placeholders::_1));
    ...
    lowCommandPublisher_ = nh->create_publisher<unitree_go::msg::LowCmd>("/lowcmd", 10); // /joint_ctrl
...
bool sairol_bridge::H1Bridge::checkExternalPublisher_(std::string topic_name)
{
...
void sairol_bridge::H1Bridge::stop()
{
    ...
    RCLCPP_INFO(nh->get_logger(), "H1 Bridge stopped.");
}

void sairol_bridge::H1Bridge::wireless_callback(unitree_go::msg::WirelessController::SharedPtr data)
{
...
void sairol_bridge::H1Bridge::lowStateHandler_(unitree_go::msg::LowState::SharedPtr msg)
{
...
void sairol_bridge::H1Bridge::publishLowCommand_()
{
    ...
    unitree_go::msg::LowCmd unitree_cmd;
    ...
    for (int i = 0; i < numJoint_; ++i)
    {
        ...
        unitree_cmd.motor_cmd[i].mode = (i < emptyJointIndex_) ? 0x0A : 0x01;
...
bool sairol_bridge::H1Bridge::initControl_(bridge_interface::msg::RobotCmd default_cmd)
{
    last_state_time_ = nh->get_clock()->now();
...
void sairol_bridge::H1Bridge::finishControl_() {
    RCLCPP_INFO(nh->get_logger(), "finishControl_ called from H1Bridge");
}
